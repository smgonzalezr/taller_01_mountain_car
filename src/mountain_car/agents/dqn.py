...
class DQNAgent:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-episode state for temporally correlated exploration (EXERCISE 3).
        # When the agent decides to explore it commits to one action for a run
        # of several steps instead of drawing a fresh action every step, so it
        # can build the sustained momentum the car needs to climb the hill.
    ...
    def select_action(self, state: np.ndarray, *, deterministic: bool = False) -> int:
        ...
        The fix (EXERCISE 3): per-step uniform sampling makes consecutive
        actions independent, so exploration averages out to a random walk that
        never escapes the valley. Instead, when the agent explores it commits
        to a single action for a run of several steps. Consecutive exploratory
        actions are then correlated in time, which is exactly what produces the
        sustained back-and-forth pumping the car needs. Greedy behaviour, used
        for evaluation and rendering, stays untouched.
        """
        if deterministic:
            with torch.no_grad():
        ...
        # Middle of an exploratory run: keep pushing the same way.
        if self._explore_steps_left > 0:
            self._explore_steps_left -= 1
            return self._explore_action

        # Start a new exploratory run: pick one action and hold it for a
        # random number of steps (a pumping motion, not per-step jitter).
        if random.random() < self.epsilon:
            self._explore_action = random.randrange(self.action_dim)
            self._explore_steps_left = random.randint(1, self.explore_repeat) - 1
            return self._explore_action
    ...
    def train(self, total_episodes: int = 500, log_interval: int = 10) -> list[float]:
        ...
        for episode in range(1, total_episodes + 1):
            ...
            # Start each episode with no exploratory run in progress.
            self._explore_steps_left = 0
